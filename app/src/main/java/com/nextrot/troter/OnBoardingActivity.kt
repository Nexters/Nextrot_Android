...
class OnBoardingActivity : AppCompatActivity(){
    ...
    private val boardings: List<OnBoarding> = listOf(
        OnBoarding(
            "트로트 듣기",
            "듣고 싶은 노래를 선택하면 한 번에<br>재생할 수 있어유 <b>쉽쥬?</b>",
            "onboarding_play.json",
            "다음",
            R.drawable.ic_paging_1),
        OnBoarding(
            "가사 보기",
            "듣고 있는 노래의 가사를 보면서<br>함께 따라 불러보셔유 <b>기가막히쥬?</b>",
    ...
    fun onClickNext(){
        if(!it.hasNext()){
            ...
            val item = it.next()
            onBoardingActivityBinding.lottieAnimationView.apply {
                setAnimation(item.lottie)
                repeatCount = 0
                playAnimation()
            }
